...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { title: 'Property Type', key: 'property_type', filterable: true,},
                { title: 'Service Type', key: 'service_type', filterable: true,},
    ...
    methods: {
        async getData() {
          this.loading = true;
          try {
            const responseData = await fetchWrapper.post(`${base_url}/admin/valuation-requests`, { 
        ...
        download() {
          const { token } = useAuthStore();
            this.loading = true;
            axios.post(`${base_url}/admin/valuation-requests/export`, {
              from_date: this.dateRange ? this.formatDate(this.dateRange[0]) : null,
              to_date: this.dateRange ? this.formatDate(this.dateRange[1]) : null,
              search_keyword: this.search,
            }, {
...
<template>
  ...
  <v-dialog v-model="viewDialog" max-width="800px">
    <v-card>
      <view-valuation-requests :selectedRow="selectedRow" @close="viewDialog = false"  />
      <v-card-actions>
  ...
  <v-dialog v-model="editDialog" max-width="800px">
    <v-card>
      <edit-valuation-requests :constants="constants" :selectedRow="selectedRow" @getData="getData()" @close="editDialog = false"/>
    </v-card>
  </v-dialog>
  ...
  <v-dialog v-model="deleteDialog" max-width="600px">
    <v-card class="p-8">
      <v-card-title class="headline">Are you sure you want to delete this valuation request?</v-card-title>
      <v-card-actions>
        <v-spacer></v-spacer>
