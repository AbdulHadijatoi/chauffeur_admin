  ...
  methods: {
    ...
    async createValuationRequest() {
      ...
      try {
        ...
        const responseData = await fetchWrapper.post(`${base_url}/admin/valuation-requests/create`, {
...
<template>
  <v-row>
    <v-col cols="12">
      <v-card elevation="0">
        <v-card-title>Create New Valuation Request</v-card-title>
        <v-card-text>
          <v-form @submit.prevent="createValuationRequest">
            <v-row dense>
              ...
              <v-col cols="6">
                <v-autocomplete 
                  ...
                  item-title="name"
                  item-value="id"
                  @update:modelValue="filterServicesCategories()"
                />
              </v-col>
              ...
              <v-col cols="6">
                <label>Based on the given Property Type, Service Type, Request Type, Vehicle and the area.</label><br><br>
