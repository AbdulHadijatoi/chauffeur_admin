...
import ViewServiceType from './ViewServicesCategories.vue';
import EditServiceType from './EditServicesCategories.vue';
import CreateServiceType from './CreateServicesCategories.vue';
import { successMessage } from '@/utils/helpers/messages';

export default {
    components: { VueDatePicker, ViewServiceType, EditServiceType, CreateServiceType },
    ...
    methods: {
        async getData() {
          this.loading = true;
          try {
            const responseData = await fetchWrapper.post(`${base_url}/admin/services-categories`, { 
        ...
        download() {
          const { token } = useAuthStore();
            this.loading = true;
            axios.post(`${base_url}/admin/services-categories/export`, {
              from_date: this.dateRange ? this.formatDate(this.dateRange[0]) : null,
              to_date: this.dateRange ? this.formatDate(this.dateRange[1]) : null,
              search_keyword: this.search,
            }, {
            ...
            .then(response => {
                ...
                link.href = url;
                link.setAttribute('download', `services-categories_${new Date().getTime()}.xlsx`);
        ...
        async deleteData(){
          this.loading = true;
          try {
            const responseData = await fetchWrapper.post(`${base_url}/admin/services-categories/delete/${this.selectedRow.id}`, { });
...
<template>
  ...
  <v-dialog v-model="viewDialog" max-width="800px">
    <v-card>
      <view-service-type :selectedRow="selectedRow" @close="viewDialog = false"  />
      <v-card-actions>
  ...
  <v-dialog v-model="editDialog" max-width="800px">
    <v-card>
      <edit-service-type @getData="getData()" :selectedRow="selectedRow" @close="editDialog = false"/>
    </v-card>
  </v-dialog>
  ...
  <v-dialog v-model="createDialog" max-width="800px">
    <v-card>
      <create-service-type @getData="getData()" @close="createDialog = false"/>
    </v-card>
  </v-dialog>
